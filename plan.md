...
**Scope**
- **First task — shrink the embedding image** (see Risk #15): it built at **16.9 GB** against a 6–8 GB estimate because `uv`'s wheel cache is retained in the same layer as the venv. Add `UV_NO_CACHE=1` (or clear the cache inside the `RUN`) to `docker/embedding.Dockerfile` and record the rebuilt size. Doing it here rather than later because this session rebuilds that image anyway to add `open_clip`, and Session 8's vLLM image + weights need the headroom.
...
15. **Image + weight disk footprint vs. available disk.** Sessions 4/5/8 pull heavy artifacts: a vLLM image (~10 GB), a torch+CUDA embedding image (~6–8 GB), 3B weights (~6 GB), and a poster cache (~1 GB) — roughly **25 GB against the 74 GB currently free**. It fits, but the WSL distro was just relocated *for space*, so budget it deliberately: pull images one session at a time and `docker system prune` stale layers between sessions rather than discovering the ceiling mid-Session-8.
    - **Measured at Session 4 — the estimate was low by ~2x.** `movie_recs-embedding:latest` is **16.9 GB**, not 6–8 GB, and free disk fell from 74 GB to **48 GB**. Cause is identified and cheap to fix: the single `uv sync` layer is 10.5 GB because `uv`'s wheel cache (`/root/.cache/uv`, ~5 GB of CUDA wheels) is written *and kept* in the same layer as the ~5 GB venv it populates. Setting `UV_NO_CACHE=1` (or `rm -rf` the cache inside the same `RUN`) should cut roughly 5 GB. **Recorded as Session 5's first task**, since Session 5 rebuilds this exact image to add `open_clip`. Revised budget if left unfixed: Session 5's rebuild replaces rather than adds (~17 GB steady-state), but Session 8's vLLM image + 3B weights (~16 GB) against 48 GB free leaves a thinner margin than the original 25-against-74 framing implied.
